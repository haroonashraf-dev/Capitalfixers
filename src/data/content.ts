export const services = [
   {
    id: "ac-repair",
    title: "AC Repair & Service",
    description: "Expert diagnostics and quick AC repairs to restore cooling. We fix leaks, do general maintenance, and deep cleaning.",
    icon: "ThermometerSnowflake",
image: "/ac-repair.webp",
    slug: "ac-repair-and-service",
    features: [ "AC General Service", "AC Installation", "AC Repairing", "AC Mounting and Dismounting", "AC Dismounting", "AC Mounting and Dismounting + AC General Service"]
  },
  {
    id: "plumbing",
    title: "Plumbing Services",
    description: "Professional plumbing solutions for leaks, pipe installations, water motor issues, and bathroom fittings.",
    icon: "Wrench",
  image: "/plumbing.webp",
    slug: "plumbing-services",
    features: ["Leak Repairs", "Pipe Installation", "Water Motor Repair", "Sanitary Fittings"]
  },
  {
    id: "electrician",
    title: "Electrician Services",
    description: "Reliable electrical troubleshooting, wiring, UPS installation, and general electrical repairs.",
    icon: "Zap",
image: "\electrician.jpg",
    slug: "electrician-services",
    features: ["Wiring & Rewiring", "UPS Installation", "Fault Finding", "Breaker Fixing"]
  },
  {
    id : "CCTV Installation",
    title: "CCTV Installation",
    description: "Professional CCTV installation for home and business security. We set up cameras, DVRs, and remote monitoring.",
    icon: "MonitorPlay",
    image: "\cctv.jpg",
    slug: "cctv-installation",
    features: ["Camera Installation", "DVR Setup", "Remote Monitoring", "System Maintenance"]
  },
  {
    id: "cleaning",
    title: "Cleaning",
    description: "Industrial-grade machines for deep cleaning of carpets and rugs right at your doorstep.",
    icon: "Brush",
    image: "\cleaning.jpeg",
    slug: "cleaning",
    features: ["Steam Cleaning", "Stain & Spot Treatment", "Quick Drying", "Dust Mite Removal"]
  },
  {
    id: "Carpenter",
    title: "Carpenter Works ",
    description: "Custom furniture making, repairs, and installations by skilled carpenters for your home or office.",
    icon: "carpenter",
    image: "\carpenter.jpg",
    slug: "carpenter-services",
    features: ["Carpenter", "Doors", "Windows", "Furniture Repair", "Custom Woodwork"]
  },
  {
    id: "fridge-repair",
    title: "Fridge & Freezer Repair",
    description: "Fast repair for refrigerators and deep freezers. We fix cooling issues, gas recharges, and compressor change.",
    icon: "Refrigerator",
    image: "/fridge.jpg",
    slug: "fridge-and-freezer-repair",
    features: ["Gas Recharging", "Compressor Replacement", "Thermostat Fix", "Defrost Issues"]
  },
  {
    id: "home-shifting",
    title: "Home Shifting Services",
    description: "Hassle-free packing, moving, and unpacking services for safe relocation across the city.",
    icon: "Truck",
    image: "/home-shifting.jpg",
    slug: "home-shifting-services",
    features: ["Professional Packing", "Safe Transport", "Unpacking Assistance", "Trained Loaders"]
  },
  {
    id: "Custom Services",
    title: "Custom Services",
    description: "Custom home maintenance and repair solutions tailored to your specific needs. Contact us for a quote.",
    image: "/custom.jpg",
    slug: "custom-services",
    features: ["Custom Home Maintenance", "Specialized Repairs", "Tailored Solutions", "Contact for Quote"]
  }
];

export const processSteps = [
  {
    id: 1,
    title: "Book Service",
    description: "Contact us via WhatsApp, Call, or our online form to schedule an appointment."
  },
  {
    id: 2,
    title: "Technician Visit",
    description: "Our certified technician will arrive on time and diagnose the AC issue safely."
  },
  {
    id: 3,
    title: "Job Done",
    description: "Expert repair or installation is completed efficiently with a satisfaction guarantee."
  }
];

export const areasServed = [
  "Islamabad", "Rawalpindi"
];

export const testimonials = [
  {
    id: 1,
    name: "Ahmad Raza",
    location: "Bahria Town Phase 7",
    review: "Incredible service! Called them for a sudden breakdown at night, and they fixed it within 2 hours. Very professional.",
    rating: 5
  },
  {
    id: 2,
    name: "Sarah Khan",
    location: "DHA Phase 2",
    review: "The technicians were polite and handled the messy deep cleaning perfectly without ruining the wall paint. Highly recommended.",
    rating: 5
  },
  {
    id: 3,
    name: "Usman Ali",
    location: "Sector G-13",
    review: "Affordable and transparent pricing. They showed me exactly what was wrong before fixing it.",
    rating: 4
  }
];

export const faqs = [
  {
    question: "How much does routine AC service cost?",
    answer: "Our standard maintenance and cleaning service starts from competitive rates depending on the AC capacity and condition. Contact us on WhatsApp for exact estimates."
  },
  {
    question: "Do you provide warranty on AC repairs?",
    answer: "Yes, we provide a standard warranty on most of our replacement parts and repair work to ensure you have peace of mind."
  },
  {
    question: "How long does a new AC installation take?",
    answer: "A standard split AC installation takes about 2-3 hours depending on the complexity of piping and site conditions."
  },
  {
    question: "What areas do you cover?",
    answer: "We primarily cover Islamabad, Rawalpindi, Bahria Town, DHA, and all nearby sectors."
  }
];
